// SWECS SwitchExternalControl&State-Power

#define AP_NAME "BDIot"
#define FW_NAME "SWECS-P"
...
#include "AdvSwitchNode.h"
#include "ACS7128Node.h"
...
#define SETTING_EXT_CONTROL_MODE 1
...
// Nodes instances
AdvSwitchNode commandNode("switch", "Switch", PIN_OUT_COMMAND, PIN_IN_EXT_STATE);
ButtonNode extControlNode("button", "Button", PIN_IN_EXT_CONTROL, INPUT_PULLUP, 10, LOW, 3, 1000);
ACS7128Node currentNode("current", "Current", 1000, 0.10f, ACS712_05B);
...
bool currentHandler(float current) {

    Homie.getLogger() << "Current sensor: " << endl
                      << BaseNode::cIndent << "I = " << current << cUnitAmpere << endl
                      << BaseNode::cIndent << "P = " << 220.0f * current << cUnitWatt << endl;
...
void setup() {
    ...
    commandNode.setOnStateChangeFunc(extStateHandler);

    currentNode.setOnChangeFunc(currentHandler);
